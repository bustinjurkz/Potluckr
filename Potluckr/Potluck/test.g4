...
prog locals[API myAPI = new API(),
    ...
    $myOutput.writeFile(rescipeArr, recipeDict, csvDict);

    $str += PeopleContainer.PeopleList.get(0).getName() + "\n";
    //$str += PeopleContainer.PeopleList.get(0).getAllergy().get(0) + "\n";
    $str += PeopleContainer.PeopleList.get(0).getDiet() + "\n";
    //$str += PeopleContainer.PeopleList.get(0).getAvailable(monthStr[PeopleContainer.PeopleList.get(0).getAvailable().get(0).getMonth()]-1).toString() + "\n";
...
allergy locals[String aller = ""]
    ...
    {
    if(!inRecipe){
        PeopleContainer.getPeopleList().get(index).addAllergy($aller);
    }
